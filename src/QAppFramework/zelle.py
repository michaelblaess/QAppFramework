"""Zell-Delegate: Innenabstand rechts und Hervorhebung des Suchbegriffs.
...
Uebernommen aus jira-timesheet-qt 0.7.2. Jede Tabelle jeder Anwendung braucht
das - ohne den Delegate klebt der Zelltext an der Spaltenkante, und zwar in
jeder Spalte, nicht nur in der letzten.
"""
...
_TREFFER_GRUND = "#ffd24a"
_TREFFER_SCHRIFT = "#1c1f24"
...
class ZellDelegate(QStyledItemDelegate):
    ...
    def setze_suchbegriff(self, begriff: str) -> None:
        """Setzt den hervorzuhebenden Suchbegriff (leer = keine Hervorhebung)."""
        self._needle = begriff

    @staticmethod
    def hervorhebung(text: str, begriff: str) -> str:
        ...
        if not begriff:
            return escaped
        muster = re.escape(begriff)
        return re.sub(
            f"({muster})",
            rf'<span style="background-color:{_TREFFER_GRUND}; '
            rf'color:{_TREFFER_SCHRIFT};">\1</span>',
            escaped,
            flags=re.IGNORECASE,
        )
    ...
    def paint(self, painter: QPainter, option: QStyleOptionViewItem, index: AnyIndex) -> None:
        ...
        inner = self.hervorhebung(elided, self._needle)
